...
<!--                        <h4 class="plan-dtl-sub-heading">{{__('staticwords.membershipline')}}</h4>-->
                        <div class="plan-dtl-list">
                            <ul>
                                <li>{{__('staticwords.membershipline')}}</li>
                    ...
                    <div class="snip1404 row">
                        ...
                        <div class="col-lg-3 col-sm-6">
                            <div class="main-plan-section  @if(isset($current_subscription) && $current_subscription->package_id == $plan->id) main-plan-section-two @endif">
                                ...
                                <div class="plan-select"><a class="btn btn-prime green-bg">{{__('staticwords.alreadysubscribe')}} </a></div>
                                <div class="plan-select"><a href="{{route('resumeSubPaypal')}}" class="btn btn-prime red-bg">{{__('staticwords.resumesubscription')}}</a></div>
                                @else
                                @if(!isset($current_subscription) && $current_subscription == NULL)
                                @if($plan->free == 1 && $plan->status == 'upcoming')
                                <div class="plan-select"><a href="#" class="btn btn-prime green-bg">{{__('COMING SOON!')}}</a></div>
                                @elseif($plan->free == 1 && $plan->status == 'active')
                                <form action="{{route('free.package.subscription',$plan->id)}}" method="POST">
                                    @csrf
                                    <div class="plan-select btn-prime-subs"><a class="btn btn-prime green-bg"><input type="submit" class="btn-subscribe" value="{{__('staticwords.subscribe')}}"></a></div>
                                </form>
                                @elseif($plan->status == 'upcoming')
                                <div class="plan-select"><a href="#" class="btn btn-prime green-bg">{{__('COMING SOON!')}}</a></div>
                                @else
                                <div class="plan-select"><a href="{{route('get_payment', $plan->id)}}" class="btn btn-prime green-bg">{{__('staticwords.subscribe')}}</a></div>
                                @endif
                                @endif
                                @endif
                                @else
