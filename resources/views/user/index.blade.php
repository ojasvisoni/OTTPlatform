          ...
          <div class="user-img">
            ...
            <div class="membership-dtl">
              ...
              @if($current_subscription != null && $method == 'stripe') 
                @if(getPlan() == 0)
                  <a href="{{route('resumeSub', $current_subscription->stripe_plan)}}" class="btn btn-prime">{{__('staticwords.resumesubscription')}}</a>
                @else
                  <a href="{{route('cancelSub', $current_subscription->stripe_plan)}}" class="btn btn-prime">{{__('staticwords.cancelsubscription')}}</a>
                @endif
              @elseif($current_subscription != null && $method == 'paypal') 
                @if($current_subscription->status == 0)
                  <a href="{{route('resumeSubPaypal')}}" class="btn btn-prime">{{__('staticwords.resumesubscription')}}</a>
                @elseif ($current_subscription->status == 1)
                  <a href="{{route('cancelSubPaypal')}}" class="btn btn-prime">{{__('staticwords.cancelsubscription')}}</a>
                @endif
              @else 
                @if($auth->is_admin != 1)
                  <a href="{{url('account/purchaseplan')}}" class="btn btn-prime">{{__('staticwords.subscribenow')}}</a>
                @endif
              @endif
          ...
          <div class="user-account-tab">
            <div id="exTab1"> 
              <ul class="nav nav-pills" id="user-tab" role="tablist">
                ...
                <li class="nav-item">
                  <a class="nav-link" id="pills-watchlist-tab"  href="{{url('account/watchlist')}}" role="tab" aria-controls="pills-watchlist" aria-selected="false"><i class="fa fa-heart"></i>{{__('Watchlist')}}</a>
                </li>
                ...
                <li class="nav-item">
                  <a class="nav-link" id="pills-affiliate-tab" data-toggle="pill" href="#affiliate" role="tab" aria-controls="pills-affiliate" aria-selected="false"><i class="fa fa-gift"></i>{{__('Affiliate Dashboard')}}</a>
                </li>
                @endif
              </ul>
                          ...
                          <tbody>
                            ...
                            @foreach($invoices as $invoice)
                              ...
                              <tr>
                                ...
                                <td>{{$from}} to {{$to}}</td>
                                <td>{{__('Stripe')}}</td>
                                <td><i class="{{$currency_symbol}}"></i> {{$invoice->amount}} @if($invoice->amount != 0) ({{ currency($invoice->amount, $from = $plan->currency, $to = Session::has('current_currency') ? ucfirst(Session::get('current_currency')) : $plan->currency, $format = true) }} {{__('equivalent to your currency')}}) @endif
                          ...
                          <tbody>
                            @foreach ($paypal_subscriptions as $item)
                              ...
                              <tr>
                                ...
                                <td><i class="{{$currency_symbol}}"></i> {{$item->price}} @if($item->price != 0) ({{ currency($item->price, $from = $item->currency, $to = Session::has('current_currency') ? ucfirst(Session::get('current_currency')) : $item->currency, $format = true) }} {{__('equivalent to your currency')}}) @endif</td>
