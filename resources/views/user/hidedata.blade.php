                                    ...
                                    @if(isset($protip) && $protip == 1)
                                        <div id="prime-next-item-description-block{{$item->movie->id}}" class="prime-description-block">
                                            ...
                                            <ul class="description-list">
                                                <li>{{__('staticwords.rating')}} {{$item->movie->rating}}</li>
                                            ...
                                            <div class="des-btn-block">
                                                ...
                                                @if($item->movie->is_upcoming != 1)
                                                    ...
                                                    @if($item->movie->maturity_rating == 'all age' || $age>=str_replace('+', '', $item->movie->maturity_rating))
                                                        ...
                                                        <a href="{{route('watchmovieiframe',$item->movie->id)}}"class="btn btn-play iframe"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                                                        </a>

                                                        @else 
                                                        <a href="{{route('watchmovie',$item->movie->id)}}" class="iframe btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span></a>
                                                        @endif
                                                    @else
                                                ...
                                                @if (isset($wishlist_check->added))
                                                    <a onclick="addWish({{$item->movie->id}},'{{$item->movie->type}}')" class="addwishlistbtn{{$item->movie->id}}{{$item->movie->type}} btn-default">{{$wishlist_check->added == 1 ? __('staticwords.removefromwatchlist') : __('staticwords.addtowatchlist')}}</a>
                                                @else
                                                    <a onclick="addWish({{$item->movie->id}},'{{$item->movie->type}}')" class="addwishlistbtn{{$item->movie->id}}{{$item->movie->type}} btn-default">{{__('staticwords.addtowatchlist')}}</a>
                                                @endif
                                ...
                                <div class="cus_img">
                                    ...
                                    @if(isset($protip) && $protip == 1)
                                        <div id="prime-next-item-description-block{{$item->season->id}}{{$item->season->type}}" class="prime-description-block">
                                            ...
                                            <div class="movie-rating">{{__('staticwords.tmdbrating')}} {{$item->season->tvseries->rating}}</div>
                                            <ul class="description-list">
                                            <li>{{__('staticwords.season')}}{{$item->season->season_no}}</li>
                                            ...
                                            @if($auth && getSubscription()->getData()->subscribed == true)
                                                ...
                                                @if($item->season->tvseries['age_req'] == 'all age' || $age>=str_replace('+', '', $item->season->tvseries['age_req']))
                                                    ...
                                                    <a href="#" onclick="playoniframe('{{ $item->season->episodes[0]->video_link['iframeurl'] }}','{{ $item->season->tvseries->id }}','tv')" class="btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                                                    </a>

                                                    @else
                                                    <a href="{{ route('watchTvShow',$item->season->id) }}" class="iframe btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span></a>
                                                    @endif
                                                @else
                                                    <a onclick="myage({{$age}})" class="btn btn-play"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                                                    </a>
                                                ...
                                                <a href="{{ route('watchtvTrailer',$item->season->id)  }}" class="iframe btn btn-default">{{__('staticwords.watchtrailer')}}</a>
                                                @endif
                                            @else
                                                @if($item->season->trailer_url != null || $item->season->trailer_url != '')
                                                <a href="{{ route('guestwatchtvtrailer',$item->season->id)  }}" class="iframe btn btn-default">{{__('staticwords.watchtrailer')}}</a>
                                                @endif
                                            @endif
                                            @if($catlog ==0 && getSubscription()->getData()->subscribed == true)
                                                @if (isset($wishlist_check->added))
                                                <a onclick="addWish({{$item->season->id}},'{{$item->season->type}}')" class="addwishlistbtn{{$item->season->id}}{{$item->season->type}} btn-default">{{$wishlist_check->added == 1 ? __('staticwords.removefromwatchlist') : __('staticwords.addtowatchlist')}}</a>
                                                @else
                                                <a onclick="addWish({{$item->season->id}},'{{$item->season->type}}')" class="addwishlistbtn{{$item->season->id}}{{$item->season->type}} btn-default">{{__('staticwords.addtowatchlist')}}
                                                </a>
                                                @endif
                                            @elseif($catlog ==1 && $auth)
                                                @if (isset($wishlist_check->added))
