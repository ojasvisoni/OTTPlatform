                ...
                <div class="row">
                    ...
                    <div class="col-md-4 mobile-view">
                        <div class="poster-thumbnail-block home-prime-slider">
                            ...
                            <a href="{{route('watchmovieiframe',$movie->id)}}" class="btn btn-play iframe mobile-play-btn"><span class="play-btn-icon"><i class="fa fa-play"></i></span> </a>
                                @else
                            <a href="{{route('watchmovie',$movie->id)}}" class="btn btn-play iframe mobile-play-btn"><span class="play-btn-icon"><i class="fa fa-play"></i></span> </a>
                            @endif
                            @else
                            <a onclick="myage({{$age}})" class="btn btn-play iframe mobile-play-btn"><span class="play-btn-icon"><i class="fa fa-play"></i></span> </a>
                            @endif
                            @endif
                            @endif
                            @endif
                        ...
                        <div class="full-movie-dtl-block">
                            ...
                            <div class="imdb-ratings-block">
                                <ul>
                                    ...
                                    @endif
                                    @endif
                                    @if(isset($movie->maturity_rating))
...
<!--                                    <li>
                            ...
                            <div id="wishlistelement" class="screen-play-btn-block ">
                                ...
                                @if($movie->maturity_rating == 'all age' || $age>=str_replace('+', '',$movie->maturity_rating))
                                @if(isset($movie->video_link['iframeurl']) && $movie->video_link['iframeurl'] != null)
                                <a href="{{route('watchmovieiframe',$movie->id)}}"class="btn btn-play iframe desktop-play-btn"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                                </a>
                                @else
                                <a href="{{route('watchmovie',$movie->id)}}" class="iframe btn btn-play desktop-play-btn"><span class="play-btn-icon"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                                </a>
                                @endif
                                @else
                                <a onclick="myage({{$age}})"  class=" btn btn-play"><span class="play-btn-icon desktop-play-btn"><i class="fa fa-play"></i></span> <span class="play-text">{{__('staticwords.playnow')}}</span>
                                </a>
                                @endif
                                @endif
                                @endif
                                ...
                                @if(in_array(1, $mlc))
                                <button type="button" class="btn btn-default watch-trailer-btn" data-toggle="collapse" data-target="#downloadmovie">{{__('staticwords.download')}}</button>
                                    ...
                                    <table  class=" text-center table table-bordered table-responsive detail-multiple-link">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td align="center">
                                                    <a data-id="{{$link->id}}" class="download btn btn-sm btn-success" title="{{__('staticwords.download')}}" href="{{$link->url}}" >
                    ...
                    <div class="full-movie-dtl-block">
                        ...
                        <div class="imdb-ratings-block">
                            <ul>
                                ...
                                @endif
                                @if(isset($season->season_no))
                                <li>{{$season->season_no}} {{__('staticwords.season')}}</li>
                                @endif
                                @if(isset($season->tvseries->age_req))
                                <li>{{$season->tvseries->age_req}}</li>
                                @endif
                                @if($configs->user_rating != 1)
                                @if(isset($season->tvseries->rating))
    ...
    <a href="{{ route('watchTrailer',$series->id)  }}" class="watch-trailer-btn iframe btn btn-default">{{__('staticwords.watchtrailer')}}</a>
    @endif
    @else
    @if($series->trailer_url != null || $series->trailer_url != '')
    <a href="{{ route('guestwatchtrailer',$series->id)  }}" class="watch-trailer-btn iframe btn btn-default">{{__('staticwords.watchtrailer')}}</a>
    @endif
    @endif
    @if($catlog ==0 && getSubscription()->getData()->subscribed == true)
    @if (isset($wishlist_check->added))
    ...
    @if(Auth::user() && getSubscription()->getData()->subscribed == true)
    @if(in_array(1, $mlc))
    <button type="button" class="btn btn-sm btn-default download-btn watch-trailer-btn" data-toggle="collapse" data-target="#downloadmovie{{$series->id}}">{{__('staticwords.download')}}</button>
    ...
    @if(Auth::user() && getSubscription()->getData()->subscribed == true)
    @if(in_array(1, $elc))
    <button type="button" class="btn btn-default download-btn watch-trailer-btn" data-toggle="collapse" data-target="#downloadtvseries{{$episode->id}}">{{__('staticwords.download')}}</button>
    ...
    <div class="container-fluid movie-series-section comment-nav-tabs">
        ...
        <div class="tab-content">
            <div role="tabpanel" class="tab-pane fade in active" id="showcomment">
                ...
                @else
                <h4 class="title" style="color:#B1B1B1;"><span class="glyphicon glyphicon-comment"></span> {{$season->tvseries->comments->where('status',1)->count()}} {{__('staticwords.comment')}} </h4>
                <br/>
            ...
            <div role="tabpanel" class="tab-pane fade" id="postcomment">
                <div style="width: 90%;color:#B1B1B1;" class=" " >
                    <h3>{{__('staticwords.postcomment')}}:</h3>
                    <br/>
