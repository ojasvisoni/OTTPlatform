...
<div class="contact-main-block container" style="background-color: #111;">
  ...
  @endif
  <h3 class="contact-us-heading text-center">{{__('staticwords.contact')}} <span
      class="us_text">{{__('staticwords.us')}}</span></h3>
  <br>
  <h5 class="contact-us-heading text-center">{{__('staticwords.contactdetail')}}</h5>
  <form class="contact-block" action="{{ route('send.contactus') }}" method="post">
    ...
    <div class="row">
      <div class="col-md-6 offset-md-3">
        ...
        <input type="submit" class="btn btn-subscribe" value="{{__('staticwords.send')}}">
      </div>
    </div>
  </form>
