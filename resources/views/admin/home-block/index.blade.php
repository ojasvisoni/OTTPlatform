...
@section('title',__('adminstaticwords.AllPromotionSettings'))
@section('content')
  <div class="content-main-block mrg-t-40">
    <div class="admin-create-btn-block">
      <a href="{{route('home-block.create')}}" class="btn btn-danger btn-md"><i class="material-icons left">add</i> {{__('adminstaticwords.AddPromotionSettingsBlock')}}</a>
      <!-- Delete Modal -->
        ...
        @if ($home_blocks)
          ...
          @foreach ($home_blocks as $key => $home_block)
            <tr id="item-{{$home_block->id}}">
              ...
              <td>{{$home_block->is_active == 1 ? __('adminstaticwords.Active') : __('adminstaticwords.Deactive')}}</td>
              <td>
