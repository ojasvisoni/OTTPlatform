...
@section('title',__('adminstaticwords.AppSlider'))
@section('content')
  <div class="content-main-block mrg-t-40">
    <div class="admin-create-btn-block">
      <a href="{{route('appslider.create')}}" class="btn btn-danger btn-md"><i class="material-icons left">add</i> {{__('adminstaticwords.AddAppSlide')}}</a>
      <!-- Delete Modal -->
    ...
    <div class="content-block box-body content-block-two">
      <table id="full_detail_table" class="table table-hover db">
        <thead>
          ...
          <th>{{__('adminstaticwords.AppSlideImage')}}</th>
        ...
        @if ($app_slides)
          ...
          @foreach ($app_slides as $key => $app_slide)
            <tr id="item-{{$app_slide->id}}">
              ...
              <td>{{$app_slide->active == 1 ? __('adminstaticwords.Active') : __('adminstaticwords.Deactive')}}</td>
              <td>
