...
<div class="admin-form-main-block">
  <h4 class="admin-form-text">
    @can('movies.view')
      <a href="{{url('admin/movies')}}" data-toggle="tooltip" data-original-title="{{__('adminstaticwords.GoBack')}}" class="btn-floating"><i class="material-icons">reply</i></a> 
    @endcan
    {{__('adminstaticwords.EditMovie')}}</h4>
  <div class="row">
    <div class="col-md-8">
...
<div id="myyoutubeModal" class="modal fade" role="dialog">
  ...
  <div class="form-group{{ $errors->has('maturity_rating') ? ' has-error' : '' }}" id="is_not_kids">
        ...
        <tr> 
          ...
          <td><button type="button" name="add" id="add" class="btn btn-xs btn-success">
            ...
            <div class="search form-group{{ $errors->has('password') ? ' has-error' : '' }} is_protect" style="{{ $movie->is_protect == 1 ? '' : 'display:none' }}" >
              ...
              <input type="password" id="password" name="password" value="{{ isset($movie->password) && $movie->password != NULL ? $movie->password : '' }}" class="form-control">
